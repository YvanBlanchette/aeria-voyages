...
export { ITEMS_PAR_PAGE } from "./cruises-config";

// ── Logos (served from public/logos) ────────────────────────────────────────
const logoRoyal = "/logos/royal.png";
const logoPrincess = "/logos/princess.png";
const logoVirgin = "/logos/virgin.png";
const logoCelebrity = "/logos/celebrity.png";
const logoNCL = "/logos/ncl.png";
const logoHAL = "/logos/hal.png";
const logoCunard = "/logos/cunard.png";
const logoSeabourn = "/logos/seabourn.png";
const logoExplora = "/logos/explora.png";

export const MESSENGER_URL = "https://m.me/aeriavoyagescroisieres";
...
export const DESTINATION_GROUPES = [
	{
		...
		destinations: ["caraibes_est", "caraibes_ouest", "caraibes_sud", "bahamas", "bermudes", "riviera_mexicaine", "canal_panama", "cote_est", "amerique_sud"],
	},
	{
...
export function buildMessengerMessage(c) {
...
export function useCroisieresMeta(initialMeta = null) {
	const [meta, setMeta] = useState(initialMeta);

	useEffect(() => {
		if (initialMeta) return; // déjà pré-rempli côté serveur
	...
	return {
		OPTS_COMPAGNIES: (meta?.compagnies ?? []).map((c) => ({ value: c, label: c })),
		OPTS_DUREES: DUREES.map((d, i) => ({ value: String(i), label: d.label })),
		OPTS_MOIS: (meta?.mois ?? []).map((m) => ({ value: String(m), label: MOIS_LONG[m].charAt(0).toUpperCase() + MOIS_LONG[m].slice(1) })),
		OPTS_ANNEES: (meta?.annees ?? []).map((a) => ({ value: a, label: a })),
...
export function useCroisieres({
	...
	const [croisieres, setCroisieres] = useState(initialData?.data ?? []);
	const [total, setTotal] = useState(initialData?.total ?? 0);
	const [chargement, setChargement] = useState(!initialData);
	// Loquet à sens unique : ne saute le fetch que tant qu'aucun fetch réel n'a
	// encore eu lieu ET qu'on est toujours sur la vue par défaut. Ne dépend pas
	// d'un simple "premier rendu" — un ref booléen naïf casse sous le double
	// appel d'effet du Strict Mode en dev (le ref survit au remount simulé).
	const hasFetched = useRef(false);

	useEffect(() => {
		const estVueParDefaut =
			!excludeUSA &&
			fDests.length === 0 &&
			fComps.length === 0 &&
			fNavires.length === 0 &&
		...
		if (!hasFetched.current && estVueParDefaut && initialData) {
			return; // déjà pré-rempli côté serveur, rien à refaire
		}
...
export async function partagerCroisiere(c, fallbackMsg) {
	...
	const itineraireTexte = ports.length > 0 ? ports.join(" → ") : c["Itinéraire"];

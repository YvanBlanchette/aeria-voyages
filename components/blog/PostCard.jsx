...
export default function PostCard({ post, lang, featured = false, priority = false }) {
	...
	return (
		<article className={`aeria-card group flex overflow-hidden ${featured ? "flex-col md:col-span-2 md:flex-row" : "flex-col"}`}>
			<div className={`relative overflow-hidden bg-[var(--ink)] ${featured ? "aspect-[16/10] md:aspect-auto md:w-1/2" : "aspect-[16/10]"}`}>
				...
				{tag && (
					<span className="absolute left-4 top-4 border border-white/15 bg-[var(--ink)]/80 px-2.5 py-1 text-[0.6875rem] uppercase tracking-[0.16em] text-[var(--text)] backdrop-blur">
						{tag.name}
					</span>
				)}
			</div>
			...
			<div className={`flex flex-1 flex-col p-6 ${featured ? "md:justify-center md:p-9" : ""}`}>
				...
				<p className="mt-3 text-[0.9375rem] leading-relaxed text-[var(--muted)]">{summarize(post, featured ? 220 : 130)}</p>

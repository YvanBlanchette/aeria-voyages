...
export default function MultiSelect({ placeholder, options, selected, onChange, groups }) {
	...
	// Si groups est fourni, construire la liste groupée
	// Sinon, afficher à plat comme avant
	const renderOptions = () => {
		if (!groups || groups.length === 0) {
			return options.map((opt) => renderOption(opt));
		}

		// Garder trace des options déjà affichées dans un groupe
		const grouped = new Set(groups.flatMap((g) => g.destinations));

		return (
			<>
				{groups.map((group) => {
					// Filtrer les options de ce groupe qui existent réellement
					const groupOpts = group.destinations
						.map((d) => options.find((o) => o.value === d))
						.filter(Boolean);
					...
					return (
						<div key={group.label}>
							...
							<div className="px-4 pt-2.5 pb-1 text-[10px] font-semibold uppercase tracking-widest text-stone-400 select-none border-t border-stone-100 first:border-t-0">
								{group.label}
							</div>
							{groupOpts.map((opt) => renderOption(opt))}
						</div>
					);
				})}

				{/* Options hors groupes (fallback) */}
				{options
					.filter((o) => !grouped.has(o.value))
	...
	const renderOption = (opt) => {
		...
		return (
			<button
				...
				disabled={isDisabled}
				className={`cursor-pointer w-full flex items-center justify-between px-4 py-2 text-sm text-left transition-colors gap-3 ${
		...
		<div className="relative w-full sm:w-auto" ref={ref}>
			<button
				...
				className={`cursor-pointer flex items-center gap-2 text-sm px-4 py-2.5 border transition-all duration-200 
					w-full sm:min-w-[215px] justify-between bg-white ${
					open || hasSelection
			...
			{open && (
				<div className="absolute top-full left-0 bg-white border border-stone-200 shadow-xl z-40 py-1.5

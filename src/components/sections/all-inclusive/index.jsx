...
export default function AllInclusivesSection() {
  ...
  const reset = () => { setDestFilter([]); setEtoiles([]); setPrix(""); setPage(1); };

  return (
    <section id="all-inclusives" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 lg:px-0">
        ...
        <div className="text-center mb-14">
          ...
          <p className="text-stone-400 tracking-wide mt-4 text-xs max-w-lg mx-auto">
            Des centaines d'hôtels tout inclus dans les Caraïbes et en Amérique centrale.
            Prix par personne, occupation double, taxes incluses.
          </p>
        ...
        <SearchMenu
          ...
          etoiles={etoiles}         setEtoiles={setEtoiles}
        ...
        {fetched && !loading && affichees.length > 0 && (
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {nbPages > 1 && (
                <span className="text-xs text-stone-400">Page {page} / {nbPages}</span>
              )}
            </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {affichees.map((f, i) => (
                <CarteAllInclusive key={`${f.id}-${i}`} forfait={f} onClick={setModalForfait} />
              ))}
            </div>
      ...
      {/* Modal */}
      {modalForfait && (
        <ModalAllInclusive forfait={modalForfait} onClose={() => setModalForfait(null)} />
      )}
    </section>
  );
}